  ...
  <div class="row" id="advanced_search_info" style="display: none;">
    <div class="col-md-12">
      ...
      <ul>
        ...
        <li><b>Words are found in the specified order and results are returned if there are at most eight words between <i>tobacco</i> and <i>regulations</i>.</b>
          <pre><code>'tobacco regulations' all words ordered distance at most 8 words</code></pre>
        </li>
        <li><b>The <code>occurs</code> keyword comes into play when more than one occurrence of a token is to be found. Varius range modifiers are available: <code>exactly</code>, <code>at least</code>, <code>at most</code>, and <code>from ... to ....</code> </b>
          <pre><code>"act" occurs at least 4 times</code></pre>
        </li>
        <li><b>The keywords <code>ftand</code>, <code>ftor</code> and <code>ftnot</code> can also be used to combine multiple query terms.</b>
          <pre><code>{ 'Medical', 'regulations' } all ordered distance at most 3 words</code></pre>
        </li>
        <li><b>The <code>window</code> keyword accepts those texts in which all keyword occur within the specified number of tokens.</b>
          <pre><code>{ 'tobacco', 'regulations' } all window 7 words</code></pre>
        </li>
        <li><b>Sometimes it is interesting to only select texts in which all searched terms occur in the <code>same sentence</code> or <code>paragraph</code>.</b>
          <pre><code>{ 'sale of tobacco', 'regulations' }  all words same sentence</code></pre>
        </li>
        <li><b>If case is insensitive, no distinction is made between characters in upper and lower case. By default, the option is <code>insensitive</code>; it can also be set to <code>sensitive</code>.</b>
          <pre><code>{ 'sale of tobacco', 'regulations' } all words using case sensitive same sentence</code></pre>
        </li>
        <li><b>If <code>stemming</code> is activated, words are shortened to a base form by a language-specific stemmer.</b>
